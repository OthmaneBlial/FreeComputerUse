...
- **Reconstruction depuis le commit `1c31407` (24 septembre 2026) :** un `git archive` propre passe `npm ci --offline`, `npm run build` et `npm pack`. Le paquet de 130 fichiers et 1 135 828 octets s’installe offline dans un projet temporaire séparé ; aucun `.env`, dossier de test, `node_modules` ni fichier SQLite n’est empaqueté. SHA-256 `3c32f40b5ae6138a5aabb167277c57fafbda4678e397f5101ac84e4e7e6e57d1`, intégrité npm `sha512-+FTjx/X9SfeS1KG2Yt/OAiJD6n2GnVFSsjf45JE8D0X7+2GrPRxkl+ok5Q/slAYgX4PbSYfH+OvaI2SzS6gSBA==`. Depuis cette installation, `agent --version`, `agent --help`, `agent doctor`, l’extraction de table depuis le CLI avec approbation TTY et le même parcours dans le dashboard réussissent ; chaque parcours utilise une action navigateur et zéro appel modèle, et les trois lignes correspondent à la table source synthétique. Le dashboard renvoie HTTP 200, passe clavier Starting page → goal, ne déborde pas à 320/390/768/1440 px et n’émet aucune erreur console. Le registre npm renvoie toujours 404 pour `free-computer-use@0.1.0`; publication et intégrité publique restent non vérifiées, donc la case demeure ouverte.
