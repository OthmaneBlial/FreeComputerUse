...
- **État intermédiaire :** une interception CDP locale s’attache aux pages avant leur première navigation. Un test sur Chrome système `154.0.8037.57` confirme qu’une redirection rapide de popup est bloquée avant toute requête cible sans approbation, puis fonctionne après approbation. Les URL contenant identifiants sont rejetées par l’API et l’agent avant création ou sauvegarde de la trace ; deux tests ciblés passent. Un proxy lié à loopback applique la permission d’origine aux requêtes HTTP, tunnels HTTPS et WebSockets ; en mode normal, il bloque les plages privées/réservées et ouvre le socket vers l’adresse numérique issue de la même résolution. Des tests simulent un premier DNS public puis une réponse loopback juste avant la connexion : ni requête HTTP ni socket tunnel n’atteint la cible. Les tests DNS couvrent les plages IPv4/IPv6 privées et réservées, IPv4 mappée, NAT64 connu, ainsi que des adresses publiques autorisées. La découverte DNS64 classifie aussi les IPv4 encapsulées dans les six préfixes RFC 6052 ; les fixtures privées sont rejetées et `8.8.8.8` est accepté. Le résolveur actuel ne renvoie que des A pour `ipv4only.arpa`, donc aucun réseau NAT64 réel n’est validé. Des tests Chrome vérifient les WebSockets approuvés/refusés et l’accès localhost explicite en Ultra ; un test Chrome `154.0.8037.57` vérifie aussi une connexion WSS autorisée, la réception d’une trame et le blocage d’une origine WSS refusée avant connexion TCP à la cible. Ce test local utilise un certificat généré et ignore son erreur ; un contrôle séparé du 24 septembre (`FCU_BROWSER_CHANNEL=chrome npm run security:wss`) se connecte à l’écho WSS public de Postman avec la validation TLS normale de Chrome et vérifie le retour d’un message synthétique. Cette preuve ne couvre qu’une destination et un navigateur. Un tunnel CONNECT en clair de type SSH est fermé sans connecter sa cible. Le profil Chromium désactive l’UDP WebRTC non relayé par le proxy ; un test Chrome local confirme qu’aucun paquet STUN n’atteint le récepteur UDP et que la préférence existante est préservée, avec un risque de panne pour les sites nécessitant de l’UDP direct. Les téléchargements rejettent un dossier destination symbolique ; les noms avec traversée Windows sont confinés et Unicode normalisé. Les imports profil n’acceptent que le schéma local ; un import invalide conserve le profil valide, les fichiers profil symboliques sont refusés et leur sauvegarde utilise un remplacement temporaire atomique en mode `0600` sur macOS. Live NAT64, protocoles non-HTTP autres que WSS et le scénario STUN, autres moteurs/versions, la matrice de systèmes et plusieurs validations restent ouverts ; ne pas cocher la phase.

- **Cas d’URL non HTTP (24 septembre 2026) :** tests agent et dashboard confirment que `file:`, `data:`, `javascript:` et `ftp:` sont rejetés avant création de trace/agent ; l’API renvoie `400` pour une URL `file:`. Un test navigateur confirme qu’une action `navigate` vers `file:///etc/passwd`, même en mode Ultra, échoue avant le changement de page. Une page locale tente aussi de naviguer vers un fichier synthétique ; Chrome reste sur l’origine autorisée et le contenu du fichier n’apparaît pas. La suite complète passe 100/100 avec Chrome système `154.0.8037.57` sur macOS `26.6` / Node `25.9.0`. Cela couvre les schémas initiaux, une action `navigate` et un essai page-vers-fichier ; pas chaque protocole déclenchable depuis une page. Les validations NAT64 réelle, autres moteurs/OS et trafic non HTTP restent ouvertes ; phase non cochée.
