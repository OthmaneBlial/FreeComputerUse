...
- **État intermédiaire (23 septembre 2026) :** un export propre du commit `1f0725d` a été extrait dans un répertoire temporaire macOS `26.6` / Apple Silicon, puis `npm ci --offline --no-audit --no-fund` a installé les 12 dépendances sans téléchargement de navigateur (`PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1`). La séquence `cp .env.example .env`, `chmod 600 .env`, `agent doctor` sans fournisseur, puis `npm run dev -- --port 0` a réussi ; le tableau de bord a répondu HTTP `200`. Sur ce checkout, `FCU_BROWSER_CHANNEL=chrome npm run ui:smoke` a aussi passé depuis un profil et un dossier de données neufs : approbation du site labo, objectif documenté `Extract the table`, une action navigateur, résultat et aperçu vérifiés, zéro appel modèle, zéro erreur console et aucun débordement à 1600 px ou 390 px. `FCU_BROWSER_CHANNEL=chrome npm run ui:results` passe avec un fournisseur de test local et un oracle indépendant ; `npm run check` passe. Cela valide ce parcours sur macOS `26.6`, Node `25.9.0` et Chrome système `154.0.8037.57` uniquement. La version minimale Node, les autres OS/navigateurs et les connexions fournisseurs en première installation restent à valider ; garder la tâche ouverte.
