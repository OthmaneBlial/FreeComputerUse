...
- **État intermédiaire (23 septembre 2026) :** `FCU_BROWSER_CHANNEL=chrome npm run ui:smoke` passe sur Node `25.9.0` et Chrome système `154.0.8037.57`, avec un dossier de données temporaire neuf et aucun fournisseur configuré. Depuis le tableau de bord, le parcours demande puis reçoit l’approbation du site labo, exécute l’objectif documenté `Extract the table`, effectue une action navigateur, vérifie le résultat et charge son aperçu ; zéro appel modèle, zéro erreur console et aucun débordement à 1600 px ou 390 px. Le smoke utilise maintenant une allowlist limitée au tableau de bord local et ne réutilise pas les workflows d’un run précédent. `FCU_BROWSER_CHANNEL=chrome npm run ui:results` passe aussi avec un fournisseur de test local, un oracle de trajet indépendant, le viewer plein écran et zéro erreur console ; `npm run check` passe. Cela ne valide ni un checkout propre, ni l’installation sur d’autres systèmes ou navigateurs, ni l’onboarding des fournisseurs. Garder la tâche ouverte.
