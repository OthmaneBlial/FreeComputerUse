...
- **État intermédiaire :** une interception CDP locale s’attache aux pages avant leur première navigation. Un test sur Chrome système `154.0.8037.57` confirme qu’une redirection rapide de popup est bloquée avant toute requête cible sans approbation, puis fonctionne après approbation. Les URL contenant identifiants sont rejetées par l’API et l’agent avant création ou sauvegarde de la trace ; deux tests ciblés passent. Un test WebSocket confirme qu’une origine non approuvée n’atteint pas le serveur. En mode normal, les noms résolus vers loopback, privé ou réservé sont rejetés avant les requêtes HTTP et WebSocket ; un test navigateur confirme le blocage malgré une permission d’origine, tandis qu’un autre vérifie que le mode Ultra conserve son accès explicite à un hôte local. Ultra et `allowExternal` contournent volontairement ces deux contrôles. Les réponses DNS ne sont toutefois pas épinglées à la connexion de Chromium, donc la course de rebinding reste ouverte. Les autres moteurs et versions, les chemins fichiers et les validations listés restent aussi ouverts ; ne pas cocher la phase.
