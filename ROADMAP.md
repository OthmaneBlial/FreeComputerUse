...
- **Parcours clavier des filtres (24 septembre 2026) :** le smoke public parcourt aussi `Real websites` vers `Practice workflows`, revient au premier onglet avec `Home`, puis utilise `Tab` et `Entrée` pour Developer, Research et Everyday. Il vérifie `aria-pressed` et les résultats attendus (4, 4 et 2 cartes). `npm run check` et le smoke complet repassent.
- **Encore ouvert :** aucun test n’a été fait avec VoiceOver/NVDA; l’arbre AX de Chrome ne prouve pas le résultat vocal ni l’ordre de lecture. Le parcours clavier complet des cartes et liens du labo et la revue du README rendu restent à faire. Le GIF montre un workflow synthétique et ne remplace pas la vidéo finale réelle de phase 8. Ne pas cocher la phase tant que ces contrôles restent ouverts.
