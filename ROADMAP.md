...
- **État intermédiaire :** `docs/LOCAL_DATA.md` décrit l’emplacement par défaut et `FCU_DATA_DIR`, le contenu sensible, l’inspection des résumés et workflows, ainsi que des commandes de copie/suppression pour macOS, Linux et Windows. Le guide rappelle que `.env` est séparé, que les données ne sont pas chiffrées et qu’aucune purge automatique n’a lieu. Sur macOS/Linux, le chargeur d’environnement refuse un `.env` symbolique et retire les droits groupe/autres avant lecture ; le test subprocess vérifie le passage de `0644` à `0600`, l’absence de clé dans stdout/stderr et la préservation des droits de la cible symbolique. `tests/profile.test.ts` vérifie les imports invalides, 40 traces conservées malgré la limite d’affichage, suppression du répertoire après arrêt puis recréation d’un historique vide, préservation octet pour octet du profil quand le système de fichiers refuse la création du fichier temporaire et reprise après interruption simulée avec un temporaire partiel (5/5). Sur Chrome `154.0.8037.57`, `VariableResolver` masque les paramètres d’URL courants de type token/API key et les jetons Bearer avant le premier enregistrement de trace et le contexte fournisseur ; le dashboard filtre aussi les états et historiques restaurés, et `agent history` masque les URL anciennes. Les tests ciblés agent, UI et CLI le vérifient. `npm run check` passe, ainsi que les suites actions/agent/profil/fournisseur/CLI (37/37) et le test dashboard URL (1/1). Un test agent vérifie aussi la suppression d’une valeur de profil issue d’une erreur fournisseur (`tests/agent.test.ts`). Sur macOS, les validations vérifient les modes `0700` pour le répertoire local, le profil navigateur et le dossier de téléchargements, et `0600` pour SQLite, préférences navigateur et fichiers téléchargés. Ces filtres restent heuristiques et les captures réelles doivent être inspectées. Le contrôle de toutes les permissions sur chaque OS et la rétention à long terme restent à faire ; ne pas cocher la phase.
