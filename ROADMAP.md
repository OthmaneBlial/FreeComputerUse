...
- **État intermédiaire :** une interception CDP locale s’attache aux pages avant leur première navigation. Un test sur Chrome système `154.0.8037.57` confirme qu’une redirection rapide de popup est bloquée avant toute requête cible sans approbation, puis fonctionne après approbation. Les URL contenant identifiants sont rejetées par l’API et l’agent avant création ou sauvegarde de la trace ; deux tests ciblés passent. Un test WebSocket confirme qu’une origine non approuvée n’atteint pas le serveur. Les variantes textuelles IPv6 se comparent maintenant par origine canonique et port exact. Les réponses DNS privées, les autres moteurs et versions, les chemins fichiers et les validations listés restent ouverts ; ne pas cocher la phase.
