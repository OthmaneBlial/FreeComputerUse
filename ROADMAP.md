...
- **Encore ouvert :** aucune annonce VoiceOver/NVDA ni aucun ordre vocal n’a pu être observé. Nouvelle tentative sur macOS `26.6` : VoiceOver Utility indique le panneau de légendes activé ; VoiceOver a été basculé temporairement à « activé », Tab a focalisé le lien d’accueil du dashboard local dans Chrome, mais aucun texte parlé n’était visible dans la capture de page ou l’état natif retourné par l’outil. VoiceOver a été restauré à « désactivé », puis le dashboard local et son onglet temporaire ont été fermés. L’arbre AX et la navigation clavier restent des preuves indirectes, pas un contrôle vocal ; la phase reste décochée jusqu’à une validation observable avec lecteur d’écran.
- **Nouvel essai (24 septembre 2026) :** dans Réglages système, le bouton VoiceOver est passé de « désactivé » à « activé » ; le panneau de légendes est coché dans VoiceOver Utility. Avec Chrome système et le dashboard local, le focus clavier a avancé du lien `FreeComputerUse home` au bouton `Local profile`. La capture de la fenêtre Chrome n’a montré aucune légende pour ces deux contrôles ; une légende apparue brièvement sur une page Nouvel onglet concernait un élément sans rapport. Je ne peux donc pas confirmer ce que VoiceOver a prononcé. VoiceOver a été restauré à « désactivé », l’onglet temporaire fermé et le serveur local arrêté. Le contrôle vocal reste ouvert jusqu’à observation fiable de l’annonce.
...
- **Réexécution après synchronisation du CLI (24 septembre 2026, commit `d978b06`) :** `env LLM_API_KEY= FCU_BROWSER_CHANNEL=chrome npm run validate` passe, avec 102/102 tests en série (156,44 s pour la suite), build, scan de sécurité (255 fichiers worktree, 257 chemins et 980 blobs historiques) et `npm audit` à zéro vulnérabilité. Aucun workflow GitHub n’est déclenché ; la vérification reste limitée à macOS `26.6`, Node `25.9.0` et Chrome système `154.0.8037.57`.
...
- **Préparation vérifiée le 24 septembre 2026 :** `package.json` déclare description, dépôt, homepage, issues, mots-clés et types ; `docs/RELEASE_CHECKLIST.md` détaille les portes npm et GitHub. Depuis un `git archive` propre du commit `d978b06`, `npm ci --offline`, `npm run build` et `npm pack` réussissent. Le tarball contient 130 fichiers pour 1 135 434 octets ; SHA-256 `eb3ff54f99ab48b1eb51db08120f636fbe3a48e1426f286e87f969ed8433b3c4`, intégrité npm `sha512-IwpNC/ybYQ2gm/QjxGgaWMjvC35QZKZWr7wzc4PvRKx2+fDEovDjrLs5VfEgMzbP7/smy4YZtAFw7kdtzOMU2Q==`. Installation offline dans un second dossier temporaire : `agent --version` correspond à `0.1.0`, `agent --help`, `agent doctor` sous Chrome système, puis `Extract the table` sur la fixture publique synthétique réussissent avec une action et zéro appel modèle. Le dashboard du même tarball répond HTTP 200 ; Chrome vérifie le passage clavier Starting page → goal, les largeurs 320/390/768/1440 px et zéro erreur console. Le manifeste du tarball inclut CLI, bibliothèque, UI, documentation et licence, sans `.env`, profil, base SQLite ou dossier de test. `npm view free-computer-use@0.1.0 version` répond encore 404 : aucune version npm publique n’est déclarée, donc la tâche reste ouverte jusqu’à publication et vérification d’intégrité du registre.
- **Reconstruction depuis le commit `9834add` (24 septembre 2026) :** un `git archive` propre passe `npm ci --offline`, `npm run build` et `npm pack`. Le paquet de 130 fichiers et 1 135 868 octets s’installe offline dans un projet temporaire séparé ; aucun `.env`, dossier de test, `node_modules` ni fichier SQLite n’est empaqueté. SHA-256 `1b9adc00cdab218391ffcbee5fe555ea5fa8a941cd52c470a383d39817ebc32c`, intégrité npm `sha512-hx5oALA9g/X/rxjVJUt9CGMQhf/5INAq1xesypyNnXp5kGCypyoW8p56xe5KKXuxTtFnEFd1TXy1vr+SFEyAQA==`. Depuis cette installation, `agent --version`, `agent --help` et `agent doctor` réussissent. Le parcours UI et le CLI avec approbation TTY exécutent `Extract the table` : chacun fait une action et zéro appel modèle, et les trois lignes extraites correspondent à `docs/lab/reports.html`. Le CLI réutilise le workflow appris dans l’UI. Le dashboard renvoie HTTP 200, passe clavier Starting page → goal, ne déborde pas à 320/390/768/1440 px et n’émet aucune erreur console. macOS `26.6`, Apple Silicon, Node `25.9.0`, Chrome système `154.0.8037.57`. Le registre npm renvoie toujours 404 pour `free-computer-use@0.1.0`; publication et intégrité publique restent non vérifiées, donc la case demeure ouverte.

#### 6.2 [x] Produire des téléchargements installables et décider du format binaire
...
- **Livrable local validé (24 septembre 2026) :** ne pas fabriquer de binaire autonome ni embarquer Chromium. Le runtime utilise `node:sqlite` et Playwright ; seul macOS `26.6`/Apple Silicon avec Node `25.9.0` et Chrome système `154.0.8037.57` est vérifié. L’archive npm versionnée de 130 fichiers et 1 135 868 octets a un `SHA256SUMS` vérifié ; l’installation depuis le fichier de release local dans un projet temporaire neuf passe `agent --version` et `agent doctor`. Le même tarball, identifié par le SHA-256 ci-dessus, passe les parcours CLI et dashboard documentés en 6.1 : extraction synthétique vérifiée, une action, zéro appel modèle, sans télécharger de navigateur ni utiliser Docker. `release-notes/0.1.0.md` indique les prérequis et commandes de revue. Aucun binaire autonome n’est annoncé ; l’asset reste un candidat local. Son upload et sa relecture depuis GitHub Release relèvent de 6.3.
