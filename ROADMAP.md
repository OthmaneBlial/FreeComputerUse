...
- **État intermédiaire :** `docs/LOCAL_DATA.md` décrit l’emplacement par défaut et `FCU_DATA_DIR`, le contenu sensible, l’inspection des résumés et workflows, ainsi que des commandes de copie/suppression pour macOS, Linux et Windows. Le guide rappelle que `.env` est séparé, que les données ne sont pas chiffrées et qu’aucune purge automatique n’a lieu. Sur macOS/Linux, le chargeur d’environnement refuse un `.env` symbolique et retire les droits groupe/autres avant lecture ; le test subprocess vérifie le passage de `0644` à `0600`, l’absence de clé dans stdout/stderr et la préservation des droits de la cible symbolique. `tests/profile.test.ts` vérifie les imports invalides, 40 traces conservées malgré la limite d’affichage, suppression du répertoire après arrêt puis recréation d’un historique vide, préservation octet pour octet du profil quand le système de fichiers refuse la création du fichier temporaire et reprise après interruption simulée avec un temporaire partiel (`tests/profile.test.ts`, 5/5). Un test ciblé sur Chrome vérifie qu’une valeur de profil levée par un fournisseur est masquée dans l’erreur, les événements de l’agent et la trace persistée (`tests/agent.test.ts`, 8/8). `npm run check` et les suites agent/profil/fournisseur passent ensemble (23/23) sur macOS/Chrome `154.0.8037.57`. Sur macOS, les validations vérifient aussi les modes `0700` pour le répertoire local, le profil navigateur et le dossier de téléchargements, et `0600` pour SQLite, préférences navigateur et fichiers téléchargés. Le contrôle de toutes les permissions sur chaque OS et la rétention à long terme restent à faire ; ne pas cocher la phase.
