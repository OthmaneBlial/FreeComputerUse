...
- **État intermédiaire :** `docs/LOCAL_DATA.md` décrit l’emplacement par défaut et `FCU_DATA_DIR`, le contenu sensible, l’inspection des résumés et workflows, ainsi que des commandes de copie/suppression pour macOS, Linux et Windows. Le guide rappelle que `.env` est séparé et que les données ne sont pas chiffrées. Les vérifications de rétention, de suppression/recréation, d’import interrompu et de permissions sur chaque OS restent à faire ; ne pas cocher la phase.
