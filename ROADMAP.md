...
- **État intermédiaire (23 septembre 2026) :** `FCU_BROWSER_CHANNEL=chrome npm run ui:results` passe avec un planificateur local de test, sans preuve de planification par modèle. L’approbation précède toute visite du labo ; l’oracle vérifie le trajet et le fichier téléchargé ; les faits du résultat, le bouton de téléchargement visible, l’affichage plein écran et le viewer passent, sans erreur console ni débordement à 390 px. Le smoke sans clé couvre aussi l’approbation et l’ouverture/fermeture du résultat au clavier. `FCU_BROWSER_CHANNEL=chrome ./node_modules/.bin/tsx --test tests/ui.test.ts` passe 9/9 sur macOS `26.6`/Apple Silicon et Chrome système `154.0.8037.57`. Quatre nouveaux scénarios UI vérifient qu’un refus ne visite pas le site, qu’un arrêt pendant la permission finit en `STOPPED`, qu’une erreur de fournisseur simulée apparaît dans le journal en `FAILED`, et qu’un critère d’extraction volontairement faux reste `FAILED` avec un résultat partiel. Le timeout est synthétique, sans requête réseau fournisseur. La révocation explicite d’un domaine approuvé et l’état UI d’un blocage de sécurité dédié restent à vérifier/implémenter ; ne pas cocher.
...
- **État intermédiaire (23 septembre 2026) :** `FCU_BROWSER_CHANNEL=chrome npm run lab:smoke` passe sur Chrome système `154.0.8037.57` : 24 pages du labo, huit cartes de parcours, logo chargé et film local réellement lu en `1600x900`. Aucun débordement horizontal à 320, 390, 768 ou 1440 px sur les pages/parcours couverts ; aucune erreur console. Le smoke a rafraîchi 50 captures du labo et deux captures du tableau de bord sous `artifacts/`. Le parcours sandbox du tableau de bord passe aussi au clavier, du formulaire au résultat ; navigation clavier complète des pages, contrastes/labels, lecteur d’écran, largeurs 320/768 du tableau de bord et revue complète des captures README restent ouverts. Les captures montrent des fixtures synthétiques ; ne pas cocher.
