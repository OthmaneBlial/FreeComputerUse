...
- **État intermédiaire :** `docs/LOCAL_DATA.md` décrit l’emplacement par défaut et `FCU_DATA_DIR`, le contenu sensible, l’inspection des résumés et workflows, ainsi que des commandes de copie/suppression pour macOS, Linux et Windows. Le guide rappelle que `.env` est séparé, que les données ne sont pas chiffrées et qu’aucune purge automatique n’a lieu. `tests/profile.test.ts` vérifie 40 traces conservées malgré la limite d’affichage, suppression du répertoire après arrêt puis recréation d’un historique vide, et préservation octet pour octet du profil quand le système de fichiers refuse la création du fichier temporaire. Sur macOS, les validations vérifient aussi les modes `0700` pour le répertoire local, le profil navigateur et le dossier de téléchargements, et `0600` pour SQLite, préférences navigateur et fichiers téléchargés. Une interruption après écriture partielle, le contrôle de toutes les permissions sur chaque OS et la rétention à long terme restent à faire ; ne pas cocher la phase.
