...
- **Préparation vérifiée le 24 septembre 2026 :** `package.json` déclare description, dépôt, homepage, issues, mots-clés et types ; `docs/RELEASE_CHECKLIST.md` détaille les portes npm et GitHub. Depuis un `git archive` propre de `77c14e7`, `npm ci --offline`, `npm run build` et `npm pack` réussissent. Le tarball contient 130 fichiers pour 1 135 380 octets ; SHA-256 `f284f114a76b41f7e5c817bd518a3bc0e7dce1c42d881e3ee40bbe8db30d17d1`, intégrité npm `sha512-bQKDRfrmkunM6geWmn9gHDIE1nLwyRP+gWjhcMWynAHWhGBd08vhRqTX8uv4s9yn/p5I6qwyhiOG6QBYPJcgCg==`. Installation offline dans un second dossier temporaire : `agent --help`, `agent doctor` sous Chrome système, puis `Extract the table` sur la fixture publique synthétique réussissent avec une action et zéro appel modèle. Le dashboard du tarball répond HTTP 200 ; Chrome vérifie le passage clavier Starting page → goal, les largeurs 320/390/768/1440 px et zéro erreur console. Aucun `.env`, profil, base SQLite ou dossier de test n’est inclus. `npm view free-computer-use version` répond encore 404 : aucune version npm publique n’est déclarée, donc la tâche reste ouverte jusqu’à publication et vérification d’intégrité du registre.
