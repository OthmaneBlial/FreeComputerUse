...
| [Claude Pro/Max via Claude Code](https://support.anthropic.com/en/articles/11145838-using-claude-code-with-your-pro-or-max-plan) | Install Claude Code 2.1.248+, sign in with `claude auth login` (without `--console`), set `LLM_PROVIDER=claude-subscription`; `LLM_MODEL` is optional |

Set `LLM_API_KEY` to the key for API services. Codex and Claude subscription modes use the account signed into their local CLIs and do not need an API key. Their local tools and MCP servers are disabled while they plan; browser actions still run in FreeComputerUse. Subscription token totals are estimates, and usage follows each plan's limits. Claude Code must be signed into the Claude account and routed through its first-party subscription service; API or Console credentials are rejected. Claude Pro/Max and ChatGPT plans have their own usage limits. OpenAI API billing is [separate from a ChatGPT subscription](https://help.openai.com/en/articles/9039756-managing-billing-for-chatgpt-and-the-api-platform); Claude API usage is [separate from a Claude Pro/Max plan](https://support.anthropic.com/en/articles/9876003-i-subscribe-to-a-paid-claude-ai-plan-why-do-i-have-to-pay-separately-for-api-usage-on-console). Anthropic API mode uses JSON output plus local schema validation because this app’s extraction schema includes dynamic fields.
