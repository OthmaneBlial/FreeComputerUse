...
- Normal mode asks before direct navigation to a new origin and blocks hostnames resolving to private or reserved address ranges, even when allowlisted. Explicit IP destinations require an exact origin grant. Ultra mode and the low-level `allowExternal` option opt out of both origin and private-address checks. A local Chrome 154 test verifies fast popup redirect denial before target receipt. DNS answers are not pinned, so rebinding races and other browser builds remain unverified. Sensitive actions have a separate confirmation gate. Ultra mode is explicit and off by default.
