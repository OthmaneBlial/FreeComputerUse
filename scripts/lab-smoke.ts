...
try{
 ...
 const metadata=await browser.page.evaluate(()=>({title:document.title,image:document.querySelector('meta[property="og:image"]')?.getAttribute('content'),imageAlt:document.querySelector('meta[property="og:image:alt"]')?.getAttribute('content'),imageWidth:document.querySelector('meta[property="og:image:width"]')?.getAttribute('content'),imageHeight:document.querySelector('meta[property="og:image:height"]')?.getAttribute('content'),twitterCard:document.querySelector('meta[name="twitter:card"]')?.getAttribute('content'),twitterImage:document.querySelector('meta[name="twitter:image"]')?.getAttribute('content')}));
 if(!metadata.title.includes('FreeComputerUse')||metadata.image!=='https://othmaneblial.github.io/FreeComputerUse/social-preview.png'||!metadata.imageAlt||metadata.imageWidth!=='1200'||metadata.imageHeight!=='630'||metadata.twitterCard!=='summary_large_image'||metadata.twitterImage!==metadata.image)throw new Error('Public site title or social preview metadata is missing: '+JSON.stringify(metadata));
 const faq=browser.page.locator('#faq details');if(await faq.count()!==4)throw new Error('Public site FAQ should have four concise answers');
 const firstAnswer=faq.first(),summary=firstAnswer.locator('summary');await summary.evaluate(element=>(element as HTMLElement).focus());await browser.page.keyboard.press('Enter');if(await firstAnswer.getAttribute('open')===null)throw new Error('FAQ answer did not open by keyboard');await browser.page.keyboard.press('Enter');if(await firstAnswer.getAttribute('open')!==null)throw new Error('FAQ answer did not close by keyboard');
