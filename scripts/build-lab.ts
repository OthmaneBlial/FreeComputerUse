...
for(const asset of ['index.html','workspace.html','lab.css','home.js','workspace.js','flows.js','examples.js'])await copyFile('lab/'+asset,'docs/lab/'+asset);
const workspace=await readFile('lab/workspace.html','utf8');
for(const screen of ['product-details','product-comparison','journey-results','itinerary','invoice-details'])await writeFile(`docs/lab/${screen}.html`,workspace.replace('<body>',`<body data-screen="${screen}">`));
