...
for(const screen of ['product-details','product-comparison','journey-results','itinerary','invoice-details','close-revenue','close-ledger','close-adjustment','close-policy','close-review'])await writeFile(`docs/lab/${screen}.html`,workspace.replace('<body>',`<body data-screen="${screen}">`));
