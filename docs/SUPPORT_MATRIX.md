...
| Node.js | `package.json` declares an install floor of Node `>=22.13.0`; this does not certify every release in that range. | The full `FCU_BROWSER_CHANNEL=chrome npm run validate` gate passed on exact minimum Node `22.13.0` on 26 September 2026: 130/130 tests, type checks, build, historical secret scan and npm audit (zero vulnerabilities). Runtime: macOS `26.6`, Apple Silicon and system Chrome `154.0.8037.57`. The same gate also passed on Node `25.9.0` on 24 September 2026. | Other Node releases and Windows/Linux have not been validated. |
