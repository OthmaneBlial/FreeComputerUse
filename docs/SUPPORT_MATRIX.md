...
| Node.js | `package.json` declares an install floor of Node `>=22.13.0`; this does not certify every release in that range. | After the HTTP cancellation fix, the full gate passed 133/133 on Node `22.13.0` and `25.9.0` on 26 September 2026. The v0.2.1 release candidate then passed `FCU_BROWSER_CHANNEL=chrome npm run validate` 134/134 on Node `25.9.0`, including type checks, build, history scan and npm audit (zero vulnerabilities). The published `free-computer-use@0.2.1` installed in a fresh prefix on Node `25.9.0`; `agent doctor` reported browser launch passed. Runtime: macOS `26.6`, Apple Silicon and system Chrome `154.0.8037.57`. | Other Node releases and Windows/Linux have not been validated. Node `22.13.0` emits the expected experimental `node:sqlite` warning. |
...
| npm package | The ESM package exposes a library and global `agent` command. | npm reports `0.2.1` as `latest` with integrity `sha512-s9I/ibu5+PyE0lmAXQfTql9Zi/S2tFCfoul2o/m/wIi3R3SdlQ1//WXLTQ/d39kpwhBqgcSqdPIhkXPWLCpetQ==`; the downloaded registry tarball matches the checked candidate byte for byte. A fresh-prefix install on Node `25.9.0` passed `agent doctor` and Chrome launch. Page reading and five-link extraction each used one browser action and zero model calls. The same checked package completed an installed-dashboard page-read task after site approval with no page errors. | Package tasks and runtime were checked on macOS `26.6`/Apple Silicon with system Chrome `154.0.8037.57`; other OS/browser combinations remain unverified. MCP package smoke covers discovery, not task execution or approvals. |
| GitHub release / archive | Public release `v0.2.1`, published 26 September 2026. | Release includes `free-computer-use-0.2.1.tgz` and `SHA256SUMS`; SHA-256 `4f97553bc2595cdca11deb2090350798ccff6c1d3e8b627247577c4385aee567` matches the npm registry download. Both assets were downloaded; the checksum passed, then the archive installed in a fresh prefix and passed `agent doctor` and a synthetic five-link extraction. | Archive requires Node.js, npm and a supported installed browser; no standalone native executable is offered. |
