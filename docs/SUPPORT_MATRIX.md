...
| --- | --- | --- | --- |
| MCP host integration | Local `stdio` server, MCP TypeScript SDK `2.0.0`; tools start, inspect, follow/verify and stop a browser task. | On 24 September 2026, four focused tests passed with the official TypeScript MCP client: bounded tool discovery, local task completion after site and sensitive-action approval, denial before a site request, invalid URL rejection and the modern stdio transport. | Manual compatibility with a desktop MCP host is not verified. Host must support form elicitation for approvals. No public npm install exists yet. |
