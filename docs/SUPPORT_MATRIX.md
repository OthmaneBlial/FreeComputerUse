...
| Node.js | `package.json` declares an install floor of Node `>=22.13.0`; this does not certify every release in that range. | After the HTTP cancellation fix, the full `FCU_BROWSER_CHANNEL=chrome npm run validate` gate passed 133/133 on Node `22.13.0` and `25.9.0` on 26 September 2026. After adding common provider-token redaction, it passed 134/134 on Node `25.9.0`; both runs include type checks, build, history scan and npm audit (zero vulnerabilities). The published `free-computer-use@0.2.0` installed in a fresh prefix on Node `22.13.0`; the `0.2.1` candidate passed fresh-prefix install and `agent doctor` on Node `25.9.0`. Runtime: macOS `26.6`, Apple Silicon and system Chrome `154.0.8037.57`. | Other Node releases and Windows/Linux have not been validated. Node `22.13.0` emits the expected experimental `node:sqlite` warning. |
| Browser engine | Playwright `1.63.0`; defaults to its matching Chromium. Optional `FCU_BROWSER_CHANNEL` selects installed Chrome/Edge to avoid a browser download. | System Chrome `154.0.8037.57` passed the full 134/134 serial suite on Node `25.9.0` after token-redaction changes. It passed 133/133 on Node `22.13.0` and `25.9.0` after proxy-cancellation changes on 26 September 2026. `FCU_BROWSER_CHANNEL=chrome npm run demo -- --headed --contact` also completed seven local actions with zero model calls and exited cleanly on 24 September 2026. | Matching Playwright Chromium is absent. Playwright warns that non-bundled browsers may be incompatible. Other browser builds and operating systems remain unverified. No browser was downloaded. |
...
| npm package | The ESM package exposes a library and global `agent` command. | The v0.2.0 registry archive had integrity `sha512-glkwOKYiU49UaD+GFiJ7IovFvEZXBkszsylVDri1Z5LlOW15H+eeerv9BEYXvIfbYiISmpsxWwsUJZIeA8UiUg==`; a published MCP command listed four tools over `stdio`. The v0.2.1 release candidate passed fresh-prefix install on Node `25.9.0`, `agent doctor` and Chrome launch; synthetic page-reading and five-link tasks each used one action and zero model calls. | Package tasks and runtime were checked on macOS `26.6`/Apple Silicon with system Chrome `154.0.8037.57`; other OS/browser combinations remain unverified. MCP package smoke covers discovery, not task execution or approvals. |
| GitHub release / archive | Versioned releases include a checksummed npm package archive. | The v0.2.0 release includes `free-computer-use-0.2.0.tgz` and `SHA256SUMS`; archive SHA-256 `dda4770ff198412180855882db6201b15cdfb25276f685854ca0dfa7124edde4` matches the npm candidate. See the versioned release page for v0.2.1 archive verification. | Archive requires Node.js, npm and a supported installed browser; no standalone native executable is offered. |
