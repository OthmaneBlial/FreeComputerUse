...
| Browser engine | Playwright `1.63.0`; defaults to its matching Chromium. Optional `FCU_BROWSER_CHANNEL` selects installed Chrome/Edge to avoid a browser download. | Installed Chrome `154.0.8037.57` launched locally and passed the focused security suite. A same-profile multi-workflow replay later ended in `SIGSEGV`; earlier synthetic download coverage had also ended in `SIGTRAP`. | Matching Playwright Chromium is absent. Do not treat system Chrome as a validated substitute; Playwright warns that non-bundled browsers may be incompatible. No browser was downloaded. |
