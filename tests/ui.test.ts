...
test('local dashboard saves a profile, executes a form, gates approval, shows metrics and replays',async()=>{
  ...
  const browser=await new Browser().launch();const errors:string[]=[];browser.page.on('pageerror',error=>errors.push(error.message));
  try{
    ...
    await browser.page.locator('#preview').waitFor({state:'visible',timeout:5000});
