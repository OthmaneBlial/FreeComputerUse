...
export class Browser {
  ...
  readonly responses: { url: string; status: number; time: number }[] = [];
  ...
  async launch() {
    if (this.options.profileDir) {
      ...
      this.context = await chromium.launchPersistentContext(folder, { headless: this.options.headless ?? true, acceptDownloads: true });
    } else {
      const engine = await chromium.launch({ headless: this.options.headless ?? true });
      this.context = await engine.newContext({ acceptDownloads: true });
    ...
    this.context.on('page', page => this.wire(page));
    this.page = this.context.pages()[0] ?? await this.context.newPage();
    this.wire(this.page);
    if (this.options.allowedOrigins?.length && !this.options.allowExternal) {
  ...
  private wire(page: Page) {
    ...
    page.on('response', response => {
      this.responses.push({ url: response.url(), status: response.status(), time: Date.now() });
