...
export function renderResults(container,trace){
  ...
  for(const [index,section] of unique.entries()){
    ...
    if(typeof section.value==='string'&&!facts(section.value).length&&section.value.length>700){const details=node('details','result-supporting');details.append(node('summary','','Read extracted page text'),content(section.value,section.format));card.append(node('p','result-prose',section.value.slice(-420)),details);}
