...
export class Agent extends EventEmitter {
  ...
  constructor(readonly options:AgentOptions){
    super();this.browser=new Browser(options.browser);this.variables=new VariableResolver(options.vault);
    this.budget=options.budget??new TokenBudget();this.workflows=new WorkflowEngine(options.store);
    this.executor=new Executor(this.browser,this.observer,this.variables,this.control,{confirmation:options.confirmation,downloadDir:options.downloadDir});
  ...
  private context(goal:string,state:PageState,completed:string[],previous?:PageState):PlanningContext{
    ...
    return {goal:this.variables.redact(goal),page:this.variables.redact(page),aliases:this.variables.aliases(),completed:completed.slice(-12),allowedOrigins:this.options.browser?.allowedOrigins??[],phase:'current batch',trustedCompletionCriteria:this.options.completionCriteria};
  }
  async run(goal:string,url?:string,providedPlan?:Plan):Promise<Trace>{
    ...
    try{
      if(!this.browser.context)await this.browser.launch();if(url)await this.browser.navigate(url);
      ...
      while(true){
        ...
        if(!plan){
          if(!this.options.provider)throw new Error('No matching local strategy/workflow; configure an LLM provider');
          this.event('PLAN','Requesting one action batch');planCalls++;
          plan=await this.options.provider.plan(this.context(goal,this.state!,completed,previous));
        }
        ...
        while(index<actions.length){
          ...
          if(!this.options.provider||repairs>=(this.options.maxRepairs??3))throw new Error(result.error??'Repair limit reached');
          ...
          const repaired=RepairSchema.parse(await this.options.provider.repair({...context,failedAction:result.action,error:result.error??'Action failed',remaining:actions.slice(index,index+12)}));
        ...
        const criteria=()=>[...plan!.completion,...(!plan!.continue?this.options.completionCriteria??[]:[])];
        ...
        if(!verification.success){
          if(!this.options.provider||repairs>=(this.options.maxRepairs??3))throw new Error('Task completion could not be verified');
          repairs++;const after=await this.observe();this.event('REPAIR','Repairing failed completion conditions');
          const repaired=RepairSchema.parse(await this.options.provider.repair({...this.context(goal,after,completed),failedAction:{type:'verification'},error:'Completion conditions failed; user-specified criteria cannot be weakened or removed',remaining:[],failedConditions:criteria()}));
          if(repaired.completion)plan.completion=repaired.completion;
          if(repaired.continue!==undefined)plan.continue=repaired.continue;
      ...
      trace.metrics={durationMs:trace.durationMs,browserActions:actions,failedActions:trace.actions.filter(a=>!a.success).length,llmCalls:calls,inputTokens:usage.inputTokens-usageStart.inputTokens,outputTokens:usage.outputTokens-usageStart.outputTokens,
        ...
        usageEstimated:trace.calls.some(c=>c.usage.estimated),provider:this.options.provider?.name??'none'};
  ...
  async replay(trace:Trace,url?:string){
    ...
    return this.run(trace.goal,url??trace.url,plan);
  }
