...
export function runtimeConfig(){
  ...
  if(!['openai-compatible','anthropic','codex-subscription'].includes(providerKind))throw new Error('LLM_PROVIDER must be openai-compatible, anthropic or codex-subscription');
  ...
  const budget=new TokenBudget({maxLLMCalls:optionalCap('FCU_MAX_LLM_CALLS'),maxInputTokens:optionalCap('FCU_MAX_INPUT_TOKENS'),maxOutputTokens:optionalCap('FCU_MAX_OUTPUT_TOKENS')},providerKind==='codex-subscription'?undefined:prices);
  const model=process.env.LLM_MODEL||(providerKind==='anthropic'||providerKind==='codex-subscription'?undefined:'deepseek-flash');
  const provider=providerKind==='codex-subscription'?new CodexSubscriptionProvider({command:process.env.CODEX_CLI_PATH,model},budget):process.env.LLM_API_KEY?new FlashProvider({key:process.env.LLM_API_KEY,model:model??'',protocol:providerKind==='anthropic'?'anthropic':'openai-chat',baseURL:process.env.LLM_BASE_URL??(providerKind==='anthropic'?'https://api.anthropic.com/v1':'https://api.deepseek.com'),format:process.env.LLM_RESPONSE_FORMAT==='json_schema'?'json_schema':'json_object'},budget):undefined;
